...
describe("scirichon-authenticator test", function(done) {
    ...
    before(() => {
        ...
        router.post('/api/test/search', async ctx => {
            ctx.body = {a:1}
        })
    ...
    it('search request no acl', async ()=>{
        const token = (await getToken('admin','admin')).token
        let response = await request.post(`/api/test/search`).set(tokenName,token)
        assert.equal(response.body.a,1)
        response = await request.post(`/api/members`).set(tokenName,token)
        assert.equal(response.body.a,1)
    })
