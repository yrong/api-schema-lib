...
const apiSchemaResponseMapper = require('api-schema-response-mapper')
const config = require('config')
const ignoreAggrMetaFields = config.get('elasticsearch.ignoreAggrMetaFields')

const getIndexByCategory = (category)=>{
    let schema_obj = apiSchema.getAncestorSchema(category),index
...
const findRefCategory = (category,key)=>{
    let refs = apiSchema.getSchemaRefProperties(category)
    for(let ref of refs){
        if(ref.attr===key){
            return ref.schema
        }
        ...
        if(!_.includes(aggsMetaFields,key)){
            if(_.isArray(val[key]['buckets'])){
                ...
                for(let internal_val of buckets){
                    ...
                    if(ref_category){
                        cached_obj = await apiSchemaCache.getItemByCategoryAndID(ref_category,internal_val.key)
...
const esResponseMapper = async function(result,params,ctx){
    if(params.aggs){
        ...
        if(result.count>0&&_.isArray(result.results)){
            result.results = await apiSchemaResponseMapper.responseMapper(result.results, params)
        }
    }
    return result
}
